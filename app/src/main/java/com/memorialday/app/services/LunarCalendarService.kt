// 纪念日 - 农历/公历转换服务
// 对应 iOS: LunarCalendarService.swift
// 使用标准农历数据表实现

package com.memorialday.app.services

import com.memorialday.app.models.LunarDate
import java.util.*

/** 农历服务 */
object LunarCalendarService {

    // 农历数据 (1900-2100)
    // 编码: bit17=闰月大小(0=29,1=30), bit16-5=12月大小(0=29,1=30), bit4-1=闰月(0=无闰)
    private val lunarInfo = intArrayOf(
        0x04bd8, 0x04ae0, 0x0a570, 0x054d5, 0x0d260, 0x0d950, 0x16554, 0x056a0, 0x09ad0, 0x055d2, // 1900-1909
        0x04ae0, 0x0a5b6, 0x0a4d0, 0x0d250, 0x1d255, 0x0b540, 0x0d6a0, 0x0ada2, 0x095b0, 0x14977, // 1910-1919
        0x04970, 0x0a4b0, 0x0b4b5, 0x06a50, 0x06d40, 0x1ab54, 0x02b60, 0x09570, 0x052f2, 0x04970, // 1920-1929
        0x06566, 0x0d4a0, 0x0ea50, 0x06e95, 0x05ad0, 0x02b60, 0x186e3, 0x092e0, 0x1c8d7, 0x0c950, // 1930-1939
        0x0d4a0, 0x1d8a6, 0x0b550, 0x056a0, 0x1a5b4, 0x025d0, 0x092d0, 0x0d2b2, 0x0a950, 0x0b557, // 1940-1949
        0x06ca0, 0x0b550, 0x15355, 0x04da0, 0x0a5b0, 0x14573, 0x052b0, 0x0a9a8, 0x0e950, 0x06aa0, // 1950-1959
        0x0aea6, 0x0ab50, 0x04b60, 0x0aae4, 0x0a570, 0x05260, 0x0f263, 0x0d950, 0x05b57, 0x056a0, // 1960-1969
        0x096d0, 0x04dd5, 0x04ad0, 0x0a4d0, 0x0d4d4, 0x0d250, 0x0d558, 0x0b540, 0x0b6a0, 0x195a6, // 1970-1979
        0x095b0, 0x049b0, 0x0a974, 0x0a4b0, 0x0b27a, 0x06a50, 0x06d40, 0x0af46, 0x0ab60, 0x09570, // 1980-1989
        0x04af5, 0x04970, 0x064b0, 0x074a3, 0x0ea50, 0x06b58, 0x055c0, 0x0ab60, 0x096d5, 0x092e0, // 1990-1999
        0x0c960, 0x0d954, 0x0d4a0, 0x0da50, 0x07552, 0x056a0, 0x0abb7, 0x025d0, 0x092d0, 0x0cab5, // 2000-2009
        0x0a950, 0x0b4a0, 0x0baa4, 0x0ad50, 0x055d9, 0x04ba0, 0x0a5b0, 0x15176, 0x052b0, 0x0a930, // 2010-2019
        0x07954, 0x06aa0, 0x0ad50, 0x05b52, 0x04b60, 0x0a6e6, 0x0a4e0, 0x0d260, 0x0ea65, 0x0d530, // 2020-2029
        0x05aa0, 0x076a3, 0x096d0, 0x04afb, 0x04ad0, 0x0a4d0, 0x1d0b6, 0x0d250, 0x0d520, 0x0dd45, // 2030-2039
        0x0b5a0, 0x056d0, 0x055b2, 0x049b0, 0x0a577, 0x0a4b0, 0x0aa50, 0x1b255, 0x06d20, 0x0ada0, // 2040-2049
        0x14b63, 0x09370, 0x049f8, 0x04970, 0x064b0, 0x168a6, 0x0ea50, 0x06aa0, 0x1a6c4, 0x0aae0, // 2050-2059
        0x0a2e0, 0x0d2e3, 0x0c960, 0x0d557, 0x0d4a0, 0x0da50, 0x05d55, 0x056a0, 0x0a6d0, 0x055d4, // 2060-2069
        0x052d0, 0x0a9b8, 0x0a950, 0x0b4a0, 0x0b6a6, 0x0ad50, 0x055a0, 0x0aba4, 0x0a5b0, 0x052b0, // 2070-2079
        0x0b273, 0x06930, 0x07337, 0x06aa0, 0x0ad50, 0x14b55, 0x04b60, 0x0a570, 0x054e4, 0x0d160, // 2080-2089
        0x0e968, 0x0d520, 0x0daa0, 0x16aa6, 0x056d0, 0x04ae0, 0x0a9d4, 0x0a4d0, 0x0d150, 0x0f252, // 2090-2099
        0x0d520                                                                                  // 2100
    )

    private val tianGan = arrayOf("甲", "乙", "丙", "丁", "戊", "己", "庚", "辛", "壬", "癸")
    private val diZhi = arrayOf("子", "丑", "寅", "卯", "辰", "巳", "午", "未", "申", "酉", "戌", "亥")
    private val lunarMonths = arrayOf("正月", "二月", "三月", "四月", "五月", "六月", "七月", "八月", "九月", "十月", "冬月", "腊月")
    private val lunarDays = arrayOf(
        "初一", "初二", "初三", "初四", "初五", "初六", "初七", "初八", "初九", "初十",
        "十一", "十二", "十三", "十四", "十五", "十六", "十七", "十八", "十九", "二十",
        "廿一", "廿二", "廿三", "廿四", "廿五", "廿六", "廿七", "廿八", "廿九", "三十"
    )

    fun lunarYearName(year: Int): String {
        val g = (year - 4) % 10; if (g < 0) return "${year}年"
        val z = (year - 4) % 12; if (z < 0) return "${year}年"
        return "${tianGan[g]}${diZhi[z]}"
    }

    fun lunarMonthName(month: Int): String = if (month in 1..12) lunarMonths[month - 1] else "${month}月"

    fun lunarDayName(day: Int): String = if (day in 1..30) lunarDays[day - 1] else "${day}日"

    /** 公历转农历 */
    fun solarToLunar(date: Date): LunarDate {
        val gregorianCal = Calendar.getInstance().apply { time = date }

        // 计算从1900年1月31日到目标日期经过的天数
        val baseCal = Calendar.getInstance().apply {
            set(1900, 0, 31, 0, 0, 0); set(Calendar.MILLISECOND, 0)
        }
        var offset = ((gregorianCal.timeInMillis - baseCal.timeInMillis) / (1000 * 60 * 60 * 24)).toInt()

        // 查找农历年
        var lunarYear = 1900
        while (lunarYear < 2101 && offset >= lunarYearDays(lunarYear)) {
            offset -= lunarYearDays(lunarYear)
            lunarYear++
        }
        if (lunarYear > 2100) lunarYear = 2100

        // 查找农历月
        val leapMonth = lunarLeapMonth(lunarYear)
        var isLeap = false
        var lunarMonth = 1
        var monthDays = lunarMonthDays(lunarYear, lunarMonth)

        while (lunarMonth <= 12 && offset >= monthDays) {
            offset -= monthDays
            if (lunarMonth == leapMonth) {
                val leapDays = lunarLeapDays(lunarYear)
                if (offset >= leapDays) {
                    offset -= leapDays
                    isLeap = true
                } else {
                    // 当前在闰月内
                    monthDays = leapDays
                    break
                }
            }
            lunarMonth++
            if (lunarMonth <= 12) monthDays = lunarMonthDays(lunarYear, lunarMonth)
        }

        if (lunarMonth > 12) {
            lunarMonth = 12
            offset = 0
        }

        val lunarDay = offset + 1
        return LunarDate(year = lunarYear, month = lunarMonth, day = lunarDay, isLeapMonth = isLeap)
    }

    /** 农历转公历 */
    fun lunarToSolar(lunar: LunarDate): Date? {
        if (lunar.year < 1900 || lunar.year > 2100) return null
        if (lunar.month < 1 || lunar.month > 12) return null
        if (lunar.day < 1 || lunar.day > 30) return null

        val baseCal = Calendar.getInstance().apply {
            set(1900, 0, 31, 0, 0, 0); set(Calendar.MILLISECOND, 0)
        }

        var totalDays = 0
        // 加上1900到目标年前一年的所有天数
        for (y in 1900 until lunar.year) {
            totalDays += lunarYearDays(y)
        }

        // 加上目标年正月的天数
        val leapMonth = lunarLeapMonth(lunar.year)

        for (m in 1 until lunar.month) {
            if (m == leapMonth) {
                totalDays += lunarLeapDays(lunar.year)
            }
            totalDays += lunarMonthDays(lunar.year, m)
        }

        if (lunar.isLeapMonth) {
            // 是闰月：先加正月的天数，再加闰月的部分天数
            totalDays += lunarMonthDays(lunar.year, lunar.month)
            totalDays += (lunar.day - 1)
        } else {
            totalDays += (lunar.day - 1)
        }

        baseCal.add(Calendar.DAY_OF_MONTH, totalDays)
        return baseCal.time
    }

    fun solarToLunarString(date: Date): String {
        val lunar = solarToLunar(date)
        return "${lunarYearName(lunar.year)}年${lunarMonthName(lunar.month)}${lunarDayName(lunar.day)}"
    }

    /** 当年农历总天数 */
    private fun lunarYearDays(y: Int): Int {
        var sum = 348 // 12个月 * 29天
        for (i in 0..14) {
            if ((lunarInfo[y - 1900] shr i) and 1 == 1) sum++
        }
        return sum + lunarLeapDays(y)
    }

    /** 闰月月份(0=无闰) */
    private fun lunarLeapMonth(y: Int): Int {
        return lunarInfo[y - 1900] and 0xf
    }

    /** 闰月天数 */
    private fun lunarLeapDays(y: Int): Int {
        return if (lunarLeapMonth(y) > 0) {
            if ((lunarInfo[y - 1900] and 0x10000) != 0) 30 else 29
        } else 0
    }

    /** 第m个月的天数 */
    private fun lunarMonthDays(y: Int, m: Int): Int {
        return if ((lunarInfo[y - 1900] and (0x10000 shr m)) != 0) 30 else 29
    }
}
