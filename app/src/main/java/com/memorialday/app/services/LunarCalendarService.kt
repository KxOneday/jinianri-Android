// 纪念日 - 农历/公历转换服务
// 对应 iOS: LunarCalendarService.swift

package com.memorialday.app.services

import com.memorialday.app.models.LunarDate
import java.util.*

/** 农历服务 */
object LunarCalendarService {

    private val chineseCalendar: Calendar
        get() {
            val cal = Calendar.getInstance(TimeZone.getTimeZone("Asia/Shanghai"))
            // Android 没有原生 Chinese calendar，使用查找表实现
            return cal
        }

    // 农历数据 (1900-2100)
    // 每个元素: bit0-3=闰月月份(0=无闰月), bit4-15=每月大小月(1=30天,0=29天)
    private val lunarInfo = intArrayOf(
        0x04bd8, 0x04ae0, 0x0a570, 0x054d5, 0x0d260, 0x0d950, 0x16554, 0x056a0, 0x09ad0, 0x055d2,
        0x04ae0, 0x0a5b6, 0x0a4d0, 0x0d250, 0x1d255, 0x0b540, 0x0d6a0, 0x0ada2, 0x095b0, 0x14977,
        0x04970, 0x0a4b0, 0x0b4b5, 0x06a50, 0x06d40, 0x1ab54, 0x02b60, 0x09570, 0x052f2, 0x04970,
        0x06566, 0x0d4a0, 0x0ea50, 0x06e95, 0x05ad0, 0x02b60, 0x186e3, 0x092e0, 0x1c8d7, 0x0c950,
        0x0d4a0, 0x1d8a6, 0x0b550, 0x056a0, 0x1a5b4, 0x025d0, 0x092d0, 0x0d2b2, 0x0a950, 0x0b557,
        0x06ca0, 0x0b550, 0x15355, 0x04da0, 0x0a5b0, 0x14573, 0x052b0, 0x0a9a8, 0x0e950, 0x06aa0,
        0x0aea6, 0x0ab50, 0x04b60, 0x0aae4, 0x0a570, 0x05260, 0x0f263, 0x0d950, 0x05b57, 0x056a0,
        0x096d0, 0x04dd5, 0x04ad0, 0x0a4d0, 0x0d4d4, 0x0d250, 0x0d558, 0x0b540, 0x0b6a0, 0x195a6,
        0x095b0, 0x049b0, 0x0a974, 0x0a4b0, 0x0b27a, 0x06a50, 0x06d40, 0x0af46, 0x0ab60, 0x09570,
        0x04af5, 0x04970, 0x064b0, 0x074a3, 0x0ea50, 0x06b58, 0x055c0, 0x0ab60, 0x096d5, 0x092e0,
        0x0c960, 0x0d954, 0x0d4a0, 0x0da50, 0x07552, 0x056a0, 0x0abb7, 0x025d0, 0x092d0, 0x0cab5,
        0x0a950, 0x0b4a0, 0x0baa4, 0x0ad50, 0x055d9, 0x04ba0, 0x0a5b0, 0x15176, 0x052b0, 0x0a930,
        0x07954, 0x06aa0, 0x0ad50, 0x05b52, 0x04b60, 0x0a6e6, 0x0a4e0, 0x0d260, 0x0ea65, 0x0d530,
        0x05aa0, 0x076a3, 0x096d0, 0x04afb, 0x04ad0, 0x0a4d0, 0x1d0b6, 0x0d250, 0x0d520, 0x0dd45,
        0x0b5a0, 0x056d0, 0x055b2, 0x049b0, 0x0a577, 0x0a4b0, 0x0aa50, 0x1b255, 0x06d20, 0x0ada0,
        0x14b63, 0x09370, 0x049f8, 0x04970, 0x064b0, 0x168a6, 0x0ea50, 0x06aa0, 0x1a6c4, 0x0aae0,
        0x0a2e0, 0x0d2e3, 0x0c960, 0x0d557, 0x0d4a0, 0x0da50, 0x05d55, 0x056a0, 0x0a6d0, 0x055d4,
        0x052d0, 0x0a9b8, 0x0a950, 0x0b4a0, 0x0b6a6, 0x0ad50, 0x055a0, 0x0aba4, 0x0a5b0, 0x052b0,
        0x0b273, 0x06930, 0x07337, 0x06aa0, 0x0ad50, 0x14b55, 0x04b60, 0x0a570, 0x054e4, 0x0d160,
        0x0e968, 0x0d520, 0x0daa0, 0x16aa6, 0x056d0, 0x04ae0, 0x0a9d4, 0x0a4d0, 0x0d150, 0x0f252,
        0x0d520
    )

    // 天干
    private val tianGan = arrayOf("甲", "乙", "丙", "丁", "戊", "己", "庚", "辛", "壬", "癸")
    // 地支
    private val diZhi = arrayOf("子", "丑", "寅", "卯", "辰", "巳", "午", "未", "申", "酉", "戌", "亥")
    // 农历月份名
    private val lunarMonths = arrayOf("正月", "二月", "三月", "四月", "五月", "六月", "七月", "八月", "九月", "十月", "冬月", "腊月")
    // 农历日名
    private val lunarDays = arrayOf(
        "初一", "初二", "初三", "初四", "初五", "初六", "初七", "初八", "初九", "初十",
        "十一", "十二", "十三", "十四", "十五", "十六", "十七", "十八", "十九", "二十",
        "廿一", "廿二", "廿三", "廿四", "廿五", "廿六", "廿七", "廿八", "廿九", "三十"
    )

    /** 农历年名称（天干地支） */
    fun lunarYearName(year: Int): String {
        val g = (year - 4) % 10
        val z = (year - 4) % 12
        return "${tianGan[g]}${diZhi[z]}"
    }

    /** 农历月份中文 */
    fun lunarMonthName(month: Int): String {
        return if (month in 1..12) lunarMonths[month - 1] else "${month}月"
    }

    /** 农历日期中文 */
    fun lunarDayName(day: Int): String {
        return if (day in 1..30) lunarDays[day - 1] else "${day}日"
    }

    /** 公历转农历 */
    fun solarToLunar(date: Date): LunarDate {
        val cal = Calendar.getInstance().apply { time = date }
        val year = cal.get(Calendar.YEAR)
        val month = cal.get(Calendar.MONTH) + 1
        val day = cal.get(Calendar.DAY_OF_MONTH)

        // 计算1900年1月31日到当前日期的天数
        val baseCal = Calendar.getInstance().apply {
            set(1900, 0, 31, 0, 0, 0)
            set(Calendar.MILLISECOND, 0)
        }
        var offset = ((cal.timeInMillis - baseCal.timeInMillis) / (1000 * 60 * 60 * 24)).toInt()

        // 查找农历年
        var lunarYear = 1900
        var daysOfYear = 0
        var i = 0
        var offset = ((cal.timeInMillis - baseCal.timeInMillis) / (1000 * 60 * 60 * 24)).toInt()
        while (i < 201) {  // 1900-2100
            daysOfYear = lunarYearDays(lunarYear)
            if (offset < daysOfYear) break
            offset -= daysOfYear
            lunarYear++
            i++
        }

        // 查找农历月
        var lunarMonth = 0
        var leap = false
        val leapMonth = lunarLeapMonth(lunarYear)
        var isLeap = false
        var temp = 0
        i = 1
        while (i <= 12 && offset > 0) {
            if (leapMonth > 0 && i == leapMonth + 1 && !leap) {
                i--
                leap = true
                temp = if (lunarLeapDays(lunarYear) > 0) 30 else 29
            } else {
                temp = if (lunarMonthDays(lunarYear, i) > 0) 30 else 29
            }
            if (leap && i == leapMonth + 1) leap = false
            offset -= temp
            if (!leap) i++
        }

        if (offset == 0 && leapMonth == i && !leap) {
            isLeap = true
        } else {
            isLeap = false
        }

        if (offset < 0) {
            offset += temp
            i--
        }

        lunarMonth = i
        val lunarDay = offset + 1

        return LunarDate(year = lunarYear, month = lunarMonth, day = lunarDay, isLeapMonth = isLeap)
    }

    /** 农历转公历 */
    fun lunarToSolar(lunar: LunarDate): Date? {
        if (lunar.year < 1900 || lunar.year > 2100) return null
        if (lunar.month < 1 || lunar.month > 12) return null
        if (lunar.day < 1 || lunar.day > 30) return null

        val baseCal = Calendar.getInstance().apply {
            set(1900, 0, 31, 0, 0, 0)
            set(Calendar.MILLISECOND, 0)
        }

        var totalDays = 0
        for (y in 1900 until lunar.year) {
            totalDays += lunarYearDays(y)
        }

        val leapMonth = lunarLeapMonth(lunar.year)
        var isLeap = false

        for (m in 1 until lunar.month) {
            if (m == leapMonth) {
                totalDays += if (lunarLeapDays(lunar.year) > 0) 30 else 29
                isLeap = true
            }
            totalDays += if (lunarMonthDays(lunar.year, m) > 0) 30 else 29
        }

        if (lunar.isLeapMonth) {
            totalDays += if (lunarMonthDays(lunar.year, lunar.month) > 0) 30 else 29
        }

        totalDays += (lunar.day - 1)

        baseCal.add(Calendar.DAY_OF_MONTH, totalDays)
        return baseCal.time
    }

    /** 公历日期 → 农历字符串 */
    fun solarToLunarString(date: Date): String {
        val lunar = solarToLunar(date)
        return "${lunarYearName(lunar.year)}年${lunarMonthName(lunar.month)}${lunarDayName(lunar.day)}"
    }

    // 私有辅助方法

    private fun lunarYearDays(y: Int): Int {
        var sum = 348  // 12 * 29
        for (i in 0..14) {
            if ((lunarInfo[y - 1900] shr i) and 1 == 1) sum += 1
        }
        return sum + lunarLeapDays(y)
    }

    private fun lunarLeapMonth(y: Int): Int {
        return lunarInfo[y - 1900] and 0xf
    }

    private fun lunarLeapDays(y: Int): Int {
        return if (lunarLeapMonth(y) > 0) {
            if ((lunarInfo[y - 1900] shr 16) and 1 == 1) 30 else 29
        } else 0
    }

    private fun lunarMonthDays(y: Int, m: Int): Int {
        return if ((lunarInfo[y - 1900] shr (16 - m)) and 1 == 1) 30 else 29
    }
}
